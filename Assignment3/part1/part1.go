...
type node struct {
	...
	open     bool // Is it in open er closed??
	...
	for len(open) != 0 {
		...
		for _, node := range neighbors {
			...
			if !nodeInSet(node, open) {
				fmt.Println("Legger til", *node)
				open = append(open, node)
				node.open = true
			}
			tempg := current.g + 1
			if tempg >= node.g { // did not find abetter path
				continue
			}
			// This path is best
			node.parent = current
...
func drawImage(board [][]node, startnode *node, stopnode *node, filename string) {
	...
	height := len(board)
	width := len(board[0])
	img := image.NewRGBA(image.Rect(0, 0, width*21+1, height*21+1))
	for i := 0; i <= img.Bounds().Max.X; i += 21 {
		for j := 0; j <= img.Bounds().Max.Y; j++ {
			img.Set(i, j, color.RGBA{0, 0, 0, 255})
		}
	}
	for j := 0; j <= img.Bounds().Max.Y; j += 21 {
		for i := 0; i <= img.Bounds().Max.X; i++ {
			img.Set(i, j, color.RGBA{0, 0, 0, 255})
		}
	}

	img.Set(10, 19, color.RGBA{255, 0, 0, 255})
