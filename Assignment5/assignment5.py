...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        # Check if we are done. Done if all variables has domains of length 1
        complete = True
        for i in assignment:
        ...
        # Pick a non-finished variable
        var = self.select_unassigned_variable(assignment)
        # Test all possible values for selected variable
        for value in assignment[var]:
            # Make a copy of assignment so we will not screw up the current assignment
            assignment_copy = copy.deepcopy(assignment)
            assignment_copy[var] = [value] # Set the domain of var to value

            # Here we want to check if this choise is arc-consistent. If so we call backtrack with the assignment_copy
    ...
    def inference(self, assignment, queue):
        ...
        while len(queue) != 0:  # While queue is not empty
            arc = queue.pop()    # Get the first element from queue
            i=arc[0]
            j=arc[1]
    ...
    def revise(self, assignment, i, j):
        ...
        """
        constraints = self.constraints[i][j]  # Get all constraints affecting i and j
        revised = False

        # Double loop through all legal values for i and j
        for i_value in assignment[i]:
            satisfied = False
            for j_value in assignment[j]:
                # If these values satisfies the constraints
                if (i_value, j_value) in constraints:
                    satisfied=True
...
def main():
    ...
    for board in boards:
        print("Going to solve ", board)
        csp = create_sudoku_csp(board)
        print_sudoku_solution(csp.backtracking_search())
        print("Numb backtrack called: ", csp.backtrack_called, " Numb of failures: ", csp.times_failure)
